...
Book* Book_new( const char *title,  const char *author,  int pages,  double price);

void Book_clear(Book *p_book);

void Book_del(Book *p_book);
...
Book* Book_new( const char    *title,
                ...
                double        price)
{
  // Book *p_book = malloc(sizeof(Book));
  Book *p_book = malloc(sizeof(*p_book));
  
  Book_init(p_book, title, author, pages, price);
  return p_book;
}
...
{
  Book_clear(p_book);
  free(p_book);       // for good safety habit
  p_book = NULL;      // IMPORTANT: caller must still set their pointer to NULL
}
