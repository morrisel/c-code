/* gcc -std=c89 -c -g -O0 -pedantic -Wall -Wextra -Werror -Wshadow classic_sort_array_lowest_to_bigest.c */
/* gcc classic_sort_array_lowest_to_bigest.o -o program -lm */
#include <stdio.h>
#include <stdbool.h>
...
int main(void)
{
	...
	puts("sort array from lowest to bigest");
	puts("------------------------------------");
	for (i=0; i<n; i++)
	{
		for (j=i; j<n; ++j)
