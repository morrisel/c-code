/* gcc -std=c89 -c -g -O0 -pedantic -Wall -Wextra -Wshadow array_cyclic_shift.c */
/* gcc array_cyclic_shift.o -o program -lm */
#include <stdio.h>
#include <stdbool.h>
...
int main(void)
{
	...
	puts("\n");
	puts("array after shifting:");
	puts("---------------------");
	printf("\tcyclic shift left:\t");
	for (i=0; i<k; i++)
	{
		int temp = arr[0];
		for (j=0; j<n; j++)
